...
check time. Server-side, the `skills-validate` WorkflowTemplate runs `alpine/git` (static
validation) and `debian:bookworm-slim` (script fixtures) — neither carries shellcheck, so
the lint stays dormant there until that template installs a matching pinned
